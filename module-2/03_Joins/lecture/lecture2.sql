...
FROM city
--JOIN state ON city.state_abbreviation 
...
FROM park
JOIN park_state ON park --
JOIN state ON park_state.state_abbreviation -- 

-- Modify the previous query to include the name of the state's capital city.

SELECT park_name, state_name, city_-- 
FROM park
JOIN park_state ON park.park_id = -- 
JOIN state ON park_state.state_abbreviation --
JOIN city ON state.capital = c--
...
SELECT state_name, --(date_established) AS --
FROM state
JOIN park_state ON state.state_abbreviation --
JOIN park ON park_state.park_id --
GROUP BY st--

-- Modify the previous query so the results include entries for all the records in the state table, even if they have no park records associated with them.

SELECT state_name, --(date_established) AS earliest_park
FROM state
LEFT JOIN park_state ON state.state_abbreviation --
LEFT JOIN park ON park_state.park_id --
GROUP BY st--
...
SELECT city_name AS place--, 'City' AS kind_of_place
FROM city
WHERE city_name LIKE 'W%'
UNION
SELECT state_name AS place--, 'State' AS kind_of_place
FROM state
...
SELECT title
FROM movie
JOIN movie_genre ON movie.movie_id --
